...
for (var name in Memory.creeps) {
	if (!Game.creeps[name]) {
		...
		if (mem.flags && mem.flags.role) {
			try{Game.flags[mem.flags.role].remove();}catch(e){
		...
		if (init.genesis === 'makeHarvester' || init.genesis === 'makeCourier') {
			var anyMiners = Game.rooms[pos.roomName].find(FIND_MY_CREEPS, {filter: function (creep) {
		...
		var spawn = pos.findClosestByRange(Object.keys(Game.spawns).map(function (name) {return Game.spawns[name]}));
